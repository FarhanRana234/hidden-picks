    ...
    <>
          ...
          <div className="hidden md:flex items-center gap-7 font-body text-sm">
            {links.map(l => (
              <Link key={l.label} to={l.to} className={`relative pb-1 transition ${isActive(l.to) ? 'text-white' : 'text-[#999] hover:text-white'}`}>
                {l.label}
                {isActive(l.to) && <span className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#FF2D78]" />}
              </Link>
            ))}
          </div>

          <div className="flex items-center gap-4">
            <Link to="/shop" className="text-[#999] hover:text-white transition">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            </Link>
            <Link to="/cart" className="relative text-[#999] hover:text-white transition">
              ...
              {count > 0 && (
                <span className="absolute -top-2 -right-2 bg-[#FF2D78] text-white text-[10px] rounded-full h-4 w-4 flex items-center justify-center font-bold">{count}</span>
              )}
            </Link>
            <a href={`https://wa.me/${WHATSAPP}`} target="_blank" rel="noopener noreferrer" className="text-[#999] hover:text-white transition">
            ...
            <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden text-white">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={menuOpen ? "M6 18L18 6M6 6l12 12" : "M4 6h16M4 12h16M4 18h16"} /></svg>
            </button>
          </div>
        </div>
      </nav>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 bg-[#0A0A0A] flex flex-col items-center justify-center gap-8 md:hidden">
          ...
          <Link to="/cart" onClick={() => setMenuOpen(false)} className="font-heading text-3xl text-white hover:text-[#FF2D78] transition">Cart</Link>
        </div>
      )}
    </>
  )
